"""
3D Gaussian Splatting Trainer for Track1_gs dataset.
...
from scene.dataset_readers_track3_mask import Dataset
...
def generate_sky_points(dataset, num_points=100000, depth_range=(50.0, 80.0), device="cuda"):
    """Specifically uses the sky mask to initialize far-field Gaussians."""
    all_points = []
    all_colors = []
    ...
    for idx in indices:
        # Only use the specific sky mask for initialization
        sky_mask = dataset.get_sky_mask(idx)
        if sky_mask is None: continue
        
        image = dataset.get_image(idx).to(device)
        pose_c2w = dataset.get_poses_torch()[idx].to(device)
        
        # Pixels where sky_mask > 0
        sky_coords = torch.where(sky_mask > 0)
...
class Trainer:
    ...
    def __init__(
        ...
        # Add sky points at random depth (may be empty if dataset has no masks)
        ...
        if num_sky == 0:
            print("[Trainer] No sky masks found in dataset, using only point cloud Gaussians")
        ...
        if num_sky > 0:
            ...
            combined_points = pc_points.to(self.device)
            combined_colors = pc_colors.to(self.device)
            combined_scales = pc_scales
        ...
        self.strategy = DefaultStrategy(
            ...
            prune_scale3d=config.get('prune_scale3d', 0.20),
            prune_scale2d=config.get('prune_scale2d', 0.90),
            ...
            reset_every=config.get('reset_every', 10000),
            verbose=True,
        )
    ...
    def train_step(self, frame_indices: np.ndarray) -> float:
        ...
        for frame_idx in frame_indices:
            ...
            # Load both masks
            sky_mask = self.dataset.get_sky_mask(frame_idx)
            person_mask = self.dataset.get_person_mask(frame_idx)
            
            # Initialize a 'keep' mask (1.0 for static background)
            loss_mask = torch.ones((1, target_image.shape[1], target_image.shape[2]), device=self.device)
            
            # Filter out Sky pixels
            if sky_mask is not None:
                loss_mask[0, sky_mask > 0] = 0.0
            
            # Filter out Person pixels
            if person_mask is not None:
                loss_mask[0, person_mask > 0] = 0.0
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'init_scale_lidar': 0.08,    # Size in meters for LiDAR points
        'init_scale_sky': 0.5,
        'init_opacity': 0.7,
        'refine_start_iter': 299,  # Start densification at iteration x
        'refine_stop_iter': 2501,  # Stop densification at iteration x
    ...
    data_dir = "/media/ee904/DATA1/sdc_reconstruction/Track3"
    output_dir = "output_track3"
    ...
    trainer.save_ply("gaussian_reconstruction_track3_mask25.ply")
    print("\nTraining complete!")
