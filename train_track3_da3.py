"""
3D Gaussian Splatting Trainer for Track3_da3 dataset.
...
def generate_sky_points(dataset, num_points=100000, depth_range=(50.0, 80.0), device="cuda"):
    ...
    for idx in indices:
        ...
        # Random depth for far-field regions
...
class Trainer:
    ...
    def __init__(
        ...
        pointcloud_file = config.get('pointcloud_file', 'Track3_lidar_da3_merge.pcd')
        self.dataset = Dataset(data_dir, pointcloud_file=pointcloud_file)
        # Pointcloud data
        ...
        sky_points, sky_colors = generate_sky_points(self.dataset, num_points=100000)
        ...
        self.strategy = DefaultStrategy(
            ...
            pause_refine_after_reset=config.get('pause_refine_after_reset', 0),
            verbose=True,
        )
    ...
    def rasterize_splats(self, camtoworld: torch.Tensor, K: torch.Tensor, width: int, height: int):
        ...
        # Get RGB colors from SH
        C0 = 0.28209479177387814
        sh0 = self.splats["sh0"].squeeze(1)
        rgb = sh0 * C0 + 0.5
