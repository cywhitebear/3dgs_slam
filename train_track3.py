...
from scene.dataset_readers_track3 import Dataset
...
def generate_sky_points(dataset, num_points=100000, depth_range=(50.0, 80.0), device="cuda"):
    ...
    if len(all_points) == 0:
        # Return empty tensors if no sky points were found
...
class Trainer:
    ...
    def __init__(
        ...
        self.strategy = DefaultStrategy(
            ...
            prune_scale3d=config.get('prune_scale3d', 0.10),
    ...
    def train_step(self, frame_indices: np.ndarray) -> float:
        ...
        for frame_idx in frame_indices:
            ...
            # Get mask if available (for Track1_gs, this will be None)
            mask = self.dataset.get_mask(frame_idx)
            if mask is not None:
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'refine_stop_iter': 3001,  # Stop densification at iteration x
    ...
    output_dir = "output_track3_all_image"
    ...
    trainer.train(num_epochs=35, batch_size=8)
    trainer.save_ply("full_data_1.ply")
    print("\nTraining complete!")
