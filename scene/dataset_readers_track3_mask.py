"""
Dataset reader for Track3 dataset.
...
class Dataset:
    def __init__(self, data_dir):
        ...
        # self.poses_file = self.data_dir / "camera_poses.csv"
        # self.intrinsics_file = self.data_dir / "camera_intrinsics.json"
        # self.pointcloud_file = self.data_dir / "Track3_test.pcd"
        ...
        self.intrinsics_file = self.data_dir / "camera_intrinsics.json"
        self.pointcloud_file = self.data_dir / "Track3_lidar_da3_merge.pcd"

        # --- UPDATED: Separate Mask Directories ---
        self.sky_mask_dir = self.data_dir / "sky_masks"
        self.person_mask_dir = self.data_dir / "person_masks"
    ...
    def _get_binary_mask(self, directory, frame_idx):
        """Helper to load a specific mask by directory and frame index."""
        if not directory.exists():
            return None
        
        timestamp_str = str(self.timestamps[self.valid_pose_indices[frame_idx]])
        mask_path = directory / f"{timestamp_str}.png"
    ...
    def get_sky_mask(self, frame_idx):
        """Returns the sky mask (255 for sky, 0 for other)."""
        return self._get_binary_mask(self.sky_mask_dir, frame_idx)

    def get_person_mask(self, frame_idx):
        """Returns the person mask (255 for person, 0 for other)."""
        return self._get_binary_mask(self.person_mask_dir, frame_idx)
